...
### Keep this outside a syncing folder

This repository lived under OneDrive for its first seven commits and should not
go back. OneDrive dehydrates files it has synced into cloud placeholders, and
some tools enumerate a directory without hydrating what they find: Playwright
reported "No tests found" for two spec files that `grep`, `esbuild` and every
editor could read perfectly well, while a byte-identical copy under a new name
was discovered immediately. A test that had been stable also began failing
around one run in three.

Nothing here works around it, because it is not a repository problem and CI
never sees it. If a tool insists a file is not there, check whether it is a
placeholder before believing your config is wrong.
